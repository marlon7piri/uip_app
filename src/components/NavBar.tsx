...
const NavBar = () => {
  ...
  return (
    <ul className='w-full flex gap-4 justify-between items-center text-2xl p-2 bg-gradient-to-r from-slate-900 to-sky-500 shadow-inner shadow-slate-900 sticky top-0 z-50'>
      ...
      <IconButton onClick={logoutAuth} className='w-10 h-10 text-sky-50 hover:bg-slate-900 rounded-full p-2'><Tooltip title='Cerrar sesión'><Logout style={{ color: '#f0f9ff' }} /></Tooltip></IconButton>
    </ul>
  )
}
